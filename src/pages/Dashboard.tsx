...
type OrderContext = {
  ...
  cost: number;
  profit: number;
  trends: { revenue: number[]; cost: number[]; profit: number[] }; // Novo: Dados para os gráficos
}

type TimeFilter = 'hoje' | 'semana' | 'mes' | 'todos';

// Componente de Gráfico Dinâmico e Tecnológico
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const fetchStats = async () => {
      ...
      try {
        ...
        // Lógica para agrupar dados por dia para os gráficos
  ...
  return (
    <motion.div initial="hidden" animate="visible" variants={containerVariants} className="space-y-10 pb-10">
      ...
      {/* MÉTRICAS FINANCEIRAS COM GRÁFICOS REAIS */}
      ...
      <motion.div variants={itemVariants} className="space-y-6">
        ...
        <motion.div variants={containerVariants} className="grid gap-4 sm:gap-6 grid-cols-2 lg:grid-cols-4">
          {[
            ...
            { label: 'Em Produção', val: stats.inProgress, icon: PackageSearch, color: 'amber' }, // Alterado de "Fazendo"
